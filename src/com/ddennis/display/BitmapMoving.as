package com.ddennis.display {
	 ...
	 * 
	 */
	public class BitmapMoving extends Sprite {
		...
		public function BitmapMoving(bmp:BitmapData, mask:BitmapData = null) {
			this._bmp = bmp;
		...
		public function reset ():void {
			...
			snapshot = new BitmapData(_bmp.width, _bmp.height);
			snapshot.draw(this);
		...
		public function move(speed:Number):void {
			...
			var cH:Number = contentBitmap.bitmapData.height;
			var n:BitmapData = new BitmapData(ct, contentBitmap.bitmapData.height);
			
			if (speed > 0) {
				
				n.copyPixels(contentBitmap.bitmapData, new Rectangle(0, cH-speed, ct, speed), zero);
				n.copyPixels(contentBitmap.bitmapData, new Rectangle(0, 0, ct, ct), new Point(0, speed));
				
				
			} else {
				speed = Math.abs(speed);
				n.copyPixels(contentBitmap.bitmapData, new Rectangle(0, cH-speed, ct, speed), new Point(0, ct - speed));
